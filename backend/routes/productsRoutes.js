...
productsRouter.put('/:productID', (req, res) => {
  ...
  let updateProductQuery = `UPDATE Products SET title="${body.title}",price="${body.price}", count="${body.count}" ,img="${body.img}",popularity="${body.popularity}",sale="${body.sale}",colors="${body.colors}", url="${body.slag}", productDesc="${body.description}" WHERE id=${productID}`;
...
productsRouter.post('/', (req, res) => {
  let body = req.body;
  let addNewProductQuery = `INSERT INTO Products (title, price, count, img, popularity, sale, colors, categoryID, productDesc, url)
          VALUES
          ("${body.title}", "${body.price}","${body.count}","${body.img}" , "${body.popularity}","${body.sale}","${body.colors}", "3" , "${body.description}" , "${body.slag}")`;
